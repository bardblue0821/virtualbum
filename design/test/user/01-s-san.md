...
#### プロフィール
- コメントタブはコメントだけが表示されている。アルバムカードも表示し、どのアルバムへのコメントか分かるように。
- 自己紹介の項目は、「自己紹介」と「URL」のみにする。他は削除する。Firstore.ruleも対応。既存データはそのままで良い。
- 自己紹介項目の設定方法が分かるよう鉛筆マークボタンを設置。
  - 配置：プロフィール欄の右上。アイコンホバー時に鉛筆マークが表示。
  - 鉛筆マークは共通コンポーネント化（新規作成）。コンポーネント名: EditPencilIcon。
  - props・className は実装者の判断で最適な設計。Button のように動作。
- プロフィール共有ボタンを作成。OS既定の共有画面を表示し、プロフィールのURLを共有できるように。
...
2. プロフィールの自己紹介項目を削る
3. プロフィール画面にプロフィールの共有ボタンを追加。
4. パスワード変更画面の作成（`/settings/password`）。
5. コメントタブでアルバム情報を表示
6. アイコンの縁の白枠を無くす。
7. URL修正。
