...
import { updateAlbum, getAlbumSafe, deleteAlbum } from "@/lib/db/repositories/album.repository";
...
export function useAlbumEdit(
  ...
  const handleChangeVisibility = useCallback(async (v: 'public' | 'friends') => {
    ...
    try {
      ...
      if (v === 'friends') {
        try {
          const { deleteRepostsByAlbum } = await import("@/lib/db/repositories/repost.repository");
